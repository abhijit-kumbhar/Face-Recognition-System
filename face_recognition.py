    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in featuers:
                ...
                cursor.execute("select Name from student1 where Student_ID="+str(id))
                n=cursor.fetchone()
                n="+".join(n)
                
                cursor.execute("select Roll_No from student1 where Student_ID="+str(id))
                r=cursor.fetchone()
                r="+".join(r)

                cursor.execute("select Student_ID from student1 where Student_ID="+str(id))
                i=cursor.fetchone()
                i="+".join(i)
